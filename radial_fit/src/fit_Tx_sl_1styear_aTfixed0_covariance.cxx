//icpc  fit_Tx_ew_1styear_aTfixed0.cxx  -I/usr/local/include/Minuit2- -I/usr/local/include/Math -L/usr/local/lib/ -lMinuit2  $CPGPLOT_FLAGS $GSL_FLAGS $MINUIT_FLAGS $CFITSIO_FLAGS -parallel -o "fit_Tx_ew_1styear_aTfixed0"
...
public:
  ...
  double operator()(double r) const {
    ...
    //    return T0*pow(1+pow(rnorm,bT),-cT/bT);
    return T0*pow(1+pow(rnorm,bT),-cT/bT);
...
public:
  Chisq(const std::vector<double>& rmid,
	...
	const Eigen::MatrixXd& CovT) : 
    rmid(rmid), rinn(rinn), rout(rout), Tx(Tx), dTx(dTx),n2spline(n2spline),CovT(CovT),fErrorDef(1.) {}
...
double Chisq::operator()(const std::vector<double>& para) const {
  ...
  double chisq=0.;
  Tsl Tsl(para[0], Tmin_fixed, factor_rcool_fixed, para[1], acool_fixed, aT_fixed, bT_fixed, para[2],n2spline);
  ...
  double Tmodel1;
  double Tmodel2;
  Eigen::MatrixXd InvCovT;
  InvCovT=CovT.inverse();

   for(ibin1=0;ibin1<Nbin;ibin1++){
    for(ibin2=0;ibin2<Nbin;ibin2++){
      Tmodel1=Tsl(rmid[ibin1]);
      Tmodel2=Tsl(rmid[ibin2]);
      chisq+=(Tmodel1-Tx[ibin1])*(Tmodel2-Tx[ibin2])*InvCovT(ibin1,ibin2);
    }}

   //  for(ibin=0;ibin<Nbin;ibin++){
   // Tmodel=Tsl(rmid[ibin]);
...
double Chisq_3d::operator()(const std::vector<double>& para) const {
  ...
  double chisq=0.;
  T3d T3d(para[0], Tmin_fixed, factor_rcool_fixed, para[1], acool_fixed, aT_fixed, bT_fixed, para[2]); 
  ...
  InvCovT=CovT.inverse();
   ...
   for(ibin1=0;ibin1<Nbin;ibin1++){
    for(ibin2=0;ibin2<Nbin;ibin2++){
      Tmodel1=T3d(rmid[ibin1]);
      Tmodel2=T3d(rmid[ibin2]);
      chisq+=(Tmodel1-Tx[ibin1])*(Tmodel2-Tx[ibin2])*InvCovT(ibin1,ibin2);
    }}
...
int main(int argc, char *argv[])
{
  if(argc!=5){
    ...
    fprintf(stderr, "      temp_cov.dat     - temperature covariance data\n");
    ...
    */
    ...
    int ibin1,ibin2;
    std::vector < double > CovLSS(Nbin*Nbin);
    char *buff=new char[10000];

    ifstream fin_cov;
    fin_cov.open(covname);
    streampos current_cov = fin_cov.tellg();
    istreambuf_iterator<char>  inF_cov(fin_cov);
    int Ncov = count( inF_cov, EOF_MARKER, '\n' );
    fin_cov.seekg(current_cov);
    char tokn[]=" ";
    char *seek;
    char *pos;
    char *p, *q;
    if(Ncov!=Nbin){
      fprintf(stderr, "ERROR Number of data != Number of covariance \n");
    }
    
    int index=0;
    for(ibin1=0;ibin1<Nbin;ibin1++){
      fin_cov.getline(buff,10000); 
    ...
    fin_cov.close();
    gsl_matrix *var = gsl_matrix_alloc(index, index);
    gsl_matrix *Covmat_boost = gsl_matrix_alloc(Nbin, Nbin);
    Eigen::MatrixXd Covmat = Eigen::MatrixXd::Zero(Nbin, Nbin);
    Eigen::MatrixXd InvCovTtmp;
    for(ibin1=0;ibin1<Nbin;ibin1++){
      for(ibin2=0;ibin2<Nbin;ibin2++){
	index=ibin1+Nbin*ibin2;
	gsl_matrix_set(Covmat_boost, ibin1, ibin2, CovLSS[index]);
	Covmat(ibin1,ibin2)=CovLSS[index];
      }}
    //InvCovTtmp=Covmat.inverse();
    //for(ibin1=0;ibin1<Nbin;ibin1++){
    //  for(ibin2=0;ibin2<Nbin;ibin2++){
    //fprintf(stderr, "%f %f\n",Covmat(ibin1,ibin2),InvCovTtmp(ibin1,ibin2));
    // }}
  ...
  //-----------------------------------------------------------------------
   ...
   //       
     ...
     Chisq_3d fFCN0(rmid,rinn,rout,Tx,dTx,n2spline,Covmat);
     ...
     //upar0.SetUpperLimit("T0", 2e+2);
     upar0.SetLowerLimit("rt", 0.0);
     //upar0.SetLowerLimit("cT", 0.0);
     ...
     Chisq fFCN(rmid,rinn,rout,Tx,dTx,n2spline,Covmat);
    ...
    //fitting w/ n2_p
     ...
     Chisq fFCN_p(rmid,rinn,rout,Tx,dTx,n2spline_p,Covmat);
     ...
     double aT_p=aT_fixed;

    //----------------------------------------------------------
    //fitting w/ n2_m

     Chisq fFCN_m(rmid,rinn,rout,Tx,dTx,n2spline_m,Covmat);
    ...
    Tsl Tsl_best(T0,Tmin,factor_rcool,rt,acool,aT,bT,cT,n2spline);
    Tsl Tsl_p(T0_p,Tmin_p,factor_rcool_p,rt_p,acool_p,aT_p,bT_p,cT_p,n2spline_p);    
    Tsl Tsl_m(T0_m,Tmin_m,factor_rcool_m,rt_m,acool_m,aT_m,bT_m,cT_m,n2spline_m);    
    ...
    std::vector<double> Tslmodel(Nbin2);  
    std::vector<double> dTslmodel(Nbin2);  
    std::vector<double> dTslmodel1(Nbin2); 
    //std::vector<double> dTslmodel1_p(Nbin2);  
    //std::vector<double> dTslmodel1_m(Nbin2);  
    std::vector<double> dTslmodel2(Nbin2);  
    ...
    for(ibin=0;ibin<Nbin2;ibin++){
      ...
      Tslmodel[ibin]=Tsl_best(rmodel[ibin]);
      dTslmodel1[ibin]=0.5*(pow(Tsl_p(rmodel[ibin])-Tslmodel[ibin],2)+pow(Tsl_m(rmodel[ibin])-Tslmodel[ibin],2));
      // dTslmodel1_p[ibin]=pow(Tsl_p(rmodel[ibin])-Tslmodel[ibin],2);
      //dTslmodel1_m[ibin]=pow(Tsl_m(rmodel[ibin])-Tslmodel[ibin],2);
      dTslmodel2[ibin]=0.;
      dTslmodel[ibin]=0.;
    }
   ...
   for(iline=0;iline<Nline;iline++){  
    ...
    cT_tmp=gsl_matrix_get (cov, 2, 0)*gauss0+gsl_matrix_get (cov, 2, 1)*gauss1+gsl_matrix_get (cov, 2, 2)*gauss2+cT;
    
    //    rcool_tmp=pow(10.,factor_rcool_tmp)*rt_tmp;

  
    //     }while(T0_tmp<0. || Tmin_tmp<0. || rt_tmp<0 || rcool_tmp<0 || rcool_tmp>rt_tmp);
     }while(T0_tmp<0. || rt_tmp<0 );

    //Tsl Tsl_tmp(T0_tmp,Tmin_tmp,factor_rcool_tmp,rt_tmp,acool_tmp,aT_tmp,bT_tmp,cT_tmp,n2spline);
    //   T3d T3d_tmp(T0_tmp,Tmin_tmp,factor_rcool_tmp,rt_tmp,acool_tmp,aT_tmp,bT_tmp,cT_tmp);     
    Tsl Tsl_tmp(T0_tmp,Tmin_fixed,factor_rcool_fixed,rt_tmp,acool_fixed,aT_fixed,bT_fixed,cT_tmp,n2spline);
    T3d T3d_tmp(T0_tmp,Tmin_fixed,factor_rcool_fixed,rt_tmp,acool_fixed,aT_fixed,bT_fixed,cT_tmp);     

    
     for(ibin=0;ibin<Nbin2;ibin++){   
       dTslmodel2[ibin]+=pow(Tsl_tmp(rmodel[ibin])-Tslmodel[ibin],2);
      }
    ...
    fprintf(fp_output,"# r[arcmin] r[Mpch_%.0f^{-1}] Tsl[keV] Tsl_m Tsl_p\n",H0);
    ...
    for(ibin=1;ibin<Nbin2;ibin++){     //ibin=0 --> rmodel=0
      dTslmodel[ibin]=sqrt(dTslmodel2[ibin]/(Nboot-1.)+dTslmodel1[ibin]); 
      //     dTslmodel[ibin]=dTslmodel1[ibin]; 
      //   dTslmodel[ibin]=sqrt(dTslmodel2[ibin]/(Nboot-1.));  
      
     fprintf(fp_output,"%.8e %.8e %.8e %.8e %.8e\n",rmodel[ibin],rmodel[ibin]*amtoMpc,Tslmodel[ibin],Tslmodel[ibin]-dTslmodel[ibin],Tslmodel[ibin]+dTslmodel[ibin]);
      //fprintf(fp_output,"%.8e %.8e %.8e %.8e %.8e\n",rmodel[ibin],rmodel[ibin]*amtoMpc,Tslmodel[ibin],Tslmodel[ibin]-dTslmodel1_m[ibin],Tslmodel[ibin]+dTslmodel1_p[ibin]);
      }
    
    for(iline=0;iline<Nline;iline++){  
            dT3dmodel[iline]=sqrt(dT3dmodel2[iline]/(Nboot-1.)+dT3dmodel1[iline]); 
      //            dT3dmodel[iline]=dT3dmodel1[iline]; 
      //dT3dmodel[iline]=sqrt(dT3dmodel2[iline]/(Nboot-1.));
      ...
      //     fprintf(fp_output2,"%.8e %.8e %.8e %.8e\n",rline[iline],T3dmodel[iline],sqrt(dT3dmodel1[iline]),sqrt(dT3dmodel2[iline]/(Nboot-1.)));   
