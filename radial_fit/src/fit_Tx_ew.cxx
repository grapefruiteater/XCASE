//icpc  fit_Tx_ew_v2.cxx  -I/usr/local/include/Minuit2- -I/usr/local/include/Math -L/usr/local/lib/ -lMinuit2  $CPGPLOT_FLAGS $GSL_FLAGS $MINUIT_FLAGS $CFITSIO_FLAGS -parallel -o "fit_Tx_ew_v2"
...
public:
  ...
  double operator()(double r) const {
    ...
    //!!!!
    //    return T0*(xT+Tmin)*pow(rnorm,-aT)*pow(1+pow(rnorm,bT),-cT/bT)/(xT+1);
    //---> error of T0 is too large
    //changed function
    return (T0*xT+Tmin)*pow(rnorm,-aT)*pow(1+pow(rnorm,bT),-cT/bT)/(xT+1);
...
double Chisq::operator()(const std::vector<double>& para) const {
  ...
  Tew Tew(para[0], para[1], para[2], para[3], acool_fixed, aT_fixed, bT_fixed, para[4],n2spline);
...
int main(int argc, char *argv[])
{
  if(argc!=6){
    fprintf(stderr, "\n");
    fprintf(stderr, " Use : ./fit_Tx_ew temp.dat n.dat acool a b\n");
    fprintf(stderr, "\n");
    ...
    fprintf(stderr, "      model : T3d= T0*( (r/rcool)^acool + Tmin)*(r/rt)^-aT*(1+(r/rt)^bT)^(-cT/bT)/((r/rcool)^acool+1)\n");
    ...
    fprintf(stderr, "      \n");
    fprintf(stderr, "      \n");
    fprintf(stderr, " e.g. : ./fit_Tx_ew temp.dat nresult/ne.dat 1 0 2\n");
    fprintf(stderr, "\n");
    return 1;
  }
  ...
  //-----------------------------------------------------------------------
   ...
   //       
     ...
     double T0=T0_prior;
     double Tmin=Tmin_prior;
     double rcool=rcool_prior;
     double rt=rt_prior;
     ...
     upar0.Add("T0", T0, 0.1);
     upar0.Add("Tmin", Tmin, 0.2);
     upar0.Add("factor_rcool", factor_rcool, 0.2);
     upar0.Add("rt", rt, 0.2);
     //     upar0.Add("acool", acool, 0.2);
     upar0.Add("cT", cT, 0.2); 
     ...
     Tmin= min0.UserState().Value("Tmin");
     factor_rcool= min0.UserState().Value("factor_rcool");
     rt= min0.UserState().Value("rt");
     //acool= min0.UserState().Value("acool");
     ...
     upar.Add("T0", T0, 1.4);
     upar.Add("Tmin", Tmin, 0.2);
     upar.Add("factor_rcool", factor_rcool, 0.2);
     upar.Add("rt", rt, 0.2);
     //upar.Add("acool", acool, 0.2);
     upar.Add("cT", cT, 0.2);

     upar.SetLowerLimit("T0", 0.);
     //upar.SetLimits("T0", 0.,20);
     upar.SetLimits("Tmin", 0.,1.);
     //upar.SetLowerLimit("Tmin", 0.);
     //upar.SetLowerLimit("acool", 0.);
     upar.SetLimits("rt", 0.,50.);
     //upar.SetLowerLimit("rt", 0.);
     //upar.SetLimits("factor_rcool", -30.,30.);
     upar.SetLimits("cT", 0.,50.);
     ...
     Tmin= min.UserState().Value("Tmin");
     factor_rcool= min.UserState().Value("factor_rcool");
     rt= min.UserState().Value("rt");
     //acool= min.UserState().Value("acool");
     cT= min.UserState().Value("cT");
   
     rcool=pow(10.,factor_rcool)*rt;
     acool=acool_fixed;
     aT=aT_fixed;
     bT=bT_fixed; 
    ...
    //fitting w/ n2_p
     ...
     upar_p.Add("Tmin", Tmin, 0.2);
     upar_p.Add("factor_rcool", factor_rcool, 0.2);
     upar_p.Add("rt", rt, 0.2);
     //     upar_p.Add("acool", acool, 0.2);
     upar_p.Add("cT", cT, 0.2);

     upar_p.SetLowerLimit("T0", 0.);
     //upar_p.SetLimits("T0", 0.,50.);
     //upar_p.SetLimits("Tmin", 0.,0.5);
     upar_p.SetLowerLimit("Tmin", 0.);
     //upar_p.SetLowerLimit("acool", 0.);
     upar_p.SetLimits("rt", 0.,50.);
     //upar_p.SetLowerLimit("rt", 0.);
     //upar_p.SetLimits("factor_rcool", -3.,0);
     upar_p.SetLimits("cT", 0.,50.);
     ...
     double Tmin_p= min_p.UserState().Value("Tmin");
     double factor_rcool_p= min_p.UserState().Value("factor_rcool");
     double rt_p= min_p.UserState().Value("rt");
     //double acool_p= min_p.UserState().Value("acool");
     double cT_p= min_p.UserState().Value("cT");
   
     double rcool_p=pow(10.,factor_rcool_p)*rt_p;
    ...
    //fitting w/ n2_m
     ...
     upar_m.Add("T0", T0_p, 1.4);
     upar_m.Add("Tmin", Tmin_p, 0.2);
     upar_m.Add("factor_rcool", factor_rcool_p, 0.2);
     upar_m.Add("rt", rt_p, 0.2);
     //upar_m.Add("acool", acool, 0.2);
     upar_m.Add("cT", cT_p, 0.2); 
    

     upar_m.SetLowerLimit("T0", 0.);
     //upar_m.SetLimits("T0", 0.,4.);
     //upar_m.SetLimits("Tmin", 0.,0.5);
     upar_m.SetLowerLimit("Tmin", 0.);
     //upar_m.SetLowerLimit("acool", 0.);
     upar_m.SetLimits("rt", 0.,50.);
     //upar_m.SetLimits("factor_rcool", -3.,0);
     upar_m.SetLimits("cT", 0.,50.);
     ...
     double Tmin_m= min_m.UserState().Value("Tmin");
     double factor_rcool_m= min_m.UserState().Value("factor_rcool");
     double rt_m= min_m.UserState().Value("rt");
     //double acool_m= min_m.UserState().Value("acool");
     double cT_m= min_m.UserState().Value("cT");
   
     double rcool_m=pow(10.,factor_rcool_m)*rt_m;
    ...
    fprintf(fp_log, " #------------------------------------------------------------------------------------------\n");
    fprintf(fp_log, "   model T3d= (T0*(r/rcool)^acool + Tmin)*(r/rt)^-aT*(1+(r/rt)^bT)^(-cT/bT)/((r/rcool)^acool+1); \n");
    fprintf(fp_log, "         rcool=(10**factor_rcool)*rt \n");

    fprintf(fp_log, "   Npara        (number of parameters)          : %d \n",Npara);
    fprintf(fp_log, "   T0           [keV]                           : %.8e \n",T0);
    fprintf(fp_log, "   Tmin                                         : %.8e \n",Tmin);
    fprintf(fp_log, "   factor_rcool                                 : %.8e \n",factor_rcool);
    ...
    fprintf(fp_log, "   rcool        [arcmin]                        : %.8e\n",rcool);
    fprintf(fp_log, " #------------------------------------------------------------------------------------------\n");
    ...
    for(ibin=0;ibin<Nbin2;ibin++){
      ...
      dTewmodel1[ibin]=0.5*(pow(Tew_m(rmodel[ibin])-Tewmodel[ibin],2)+pow(Tew_p(rmodel[ibin])-Tewmodel[ibin],2));
      dTewmodel2[ibin]=0.;
      dTewmodel[ibin]=0.;
    }
   ...
   for(iline=0;iline<Nline;iline++){  
     ...
     double T0_tmp;
     double Tmin_tmp;
     double factor_rcool_tmp;
     double rt_tmp;
     double cT_tmp=10.;
   
     double rcool_tmp;
     double acool_tmp=acool_fixed;
     double aT_tmp=aT_fixed;
    ...
    Tmin_tmp  =gsl_matrix_get (cov, 1, 0)*gauss0+gsl_matrix_get (cov, 1, 1)*gauss1+Tmin;
    factor_rcool_tmp=gsl_matrix_get (cov, 2, 0)*gauss0+gsl_matrix_get (cov, 2, 1)*gauss1+gsl_matrix_get (cov, 2, 2)*gauss2+factor_rcool;
    rt_tmp=gsl_matrix_get (cov, 3, 0)*gauss0+gsl_matrix_get (cov, 3, 1)*gauss1+gsl_matrix_get (cov, 3, 2)*gauss2+gsl_matrix_get (cov, 3, 3)*gauss3+rt;
    //cT_tmp  =gsl_matrix_get (cov, 4, 0)*gauss0+gsl_matrix_get (cov, 4, 1)*gauss1+gsl_matrix_get (cov, 4, 2)*gauss2+gsl_matrix_get (cov, 4, 3)*gauss3+gsl_matrix_get (cov, 4, 4)*gauss4+cT;
    
    rcool_tmp=pow(10.,factor_rcool_tmp)*rt_tmp;

  
    //   }while(T0_tmp<0. || Tmin_tmp<0. || rt_tmp<0 || cT_tmp<0 || rcool_tmp<0 || rcool_tmp>rt_tmp);
     }while(T0_tmp<0.|| Tmin_tmp<0. || rt_tmp<0 || rcool_tmp<0 || cT_tmp<0);
    //cout<<T0_tmp<<" "<< Tmin_tmp<<" "<<rt_tmp<<" "<<cT_tmp<<" "<<rcool_tmp <<endl;
    Tew Tew_tmp(T0_tmp,Tmin_tmp,factor_rcool_tmp,rt_tmp,acool_tmp,aT_tmp,bT_tmp,cT_tmp,n2spline);
