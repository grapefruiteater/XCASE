//icpc  fit_sx_beta_mos1+mos2+pn.cxx  -I/usr/local/include/Minuit2 -I/usr/local/include/Math -L/usr/local/lib/ -lMinuit2  $CPGPLOT_FLAGS $GSL_FLAGS $MINUIT_FLAGS $CFITSIO_FLAGS -parallel -o "fit_sx_beta_mos1+mos2+pn"
...
double Chisq::operator()(const std::vector<double>& para) const {
  ...
  double sb_mos1=para[3];
  double sb_mos2=para[4];
  double sb_pn=para[5];
...
if(argc!=24){
 ...
 //-----------------------------------------------------------------------
  ...
  for(ibin=0;ibin<Nbin;ibin++){
    ...
    dsx_sys0[ibin]=sqrt((pow(sx_mos10[ibin]-sx_ave[ibin]*A_mos1[0]*con_mos1,2)+pow(sx_mos20[ibin]-sx_ave[ibin]*Amos2_norm*con_mos2,2)+pow(sx_pn0[ibin]-sx_ave[ibin]*A_pn[0]*con_pn,2))/3./2.);
   ...
   //========================================================================================
    ...
    fprintf(fp_log, "      norm2        =n02^2                          : %.8e \n",norm2);
    fprintf(fp_log, "      r02          [pix]                           : %.8e \n",r02);
    fprintf(fp_log, "      r02          [arcmin]                        : %.8e \n",r02*pixtoam);
    fprintf(fp_log, "      beta2                                        : %.8e \n",beta2);
    fprintf(fp_log, "      gamma(fix)                                   : %.8e \n",gamma1);
    fprintf(fp_log, "      epsilon(fix)                                 : %.8e \n",epsilon);
    fprintf(fp_log, "  3:  sb_mos1                                      : %.8e \n",sb_mos1);
    fprintf(fp_log, "  4:  sb_mos2                                      : %.8e \n",sb_mos2);
    fprintf(fp_log, "  5:  sb_pn                                        : %.8e \n",sb_pn);
    fprintf(fp_log, "#------------------------------------------------------------------------------------------\n");
    ...
    // best fit
    //=================================================================================================================
    Sx_beta Sx_mos1(norm1,r01,beta1);
    n_beta n_Vikhlinin0(norm1,r01,beta1);
    n2_beta n2_Vikhlinin0(norm1,r01,beta1);
  ...
  for(iboot=0;iboot<Nboot;iboot++){
    ...
    }while(r01_tmp<0. || norm1_tmp<0. || beta1_tmp<0. );
