//icpc  fit_Tx_ew_1styear_aTfixed0_cTfixed0.cxx  -I/usr/local/include/Minuit2- -I/usr/local/include/Math -L/usr/local/lib/ -lMinuit2  $CPGPLOT_FLAGS $GSL_FLAGS $MINUIT_FLAGS $CFITSIO_FLAGS -parallel -o "fit_Tx_ew_1styear_aTfixed0_cTfixed0"
...
double Chisq::operator()(const std::vector<double>& para) const {
  ...
  double chisq=0.;
  //  Tsl Tsl(para[0], para[1], para[2], para[3], para[4], aT_fixed, bT_fixed, para[5],n2spline);
  //  Tsl Tsl(para[0], Tmin_fixed, factor_rcool_fixed, para[1], acool_fixed, aT_fixed, bT_fixed, para[2],n2spline);
  Tsl Tsl(para[0], Tmin_fixed, factor_rcool_fixed, para[1], acool_fixed, aT_fixed, bT_fixed, cT_fixed,n2spline);
...
double Chisq_3d::operator()(const std::vector<double>& para) const {
  ...
  T3d T3d(para[0], Tmin_fixed, factor_rcool_fixed, para[1], acool_fixed, aT_fixed, bT_fixed, cT_fixed); 
  int ibin;
  int ibin1;
  int ibin2;
  int Nbin=rmid.size();
...
int main(int argc, char *argv[])
{
  if(argc!=6){
   ...
   if(stat("Tresult", &st)!=0){
    ...
    cT_fixed=atof(argv[4]);
    sprintf(covname,"%s",argv[5]);
    /*
  ...
  //-----------------------------------------------------------------------
   ...
   for(iline=0;iline<Nline;iline++){  
    ...
    Tsl Tsl_tmp(T0_tmp,Tmin_fixed,factor_rcool_fixed,rt_tmp,acool_fixed,aT_fixed,bT_fixed,cT_fixed,n2spline);
    T3d T3d_tmp(T0_tmp,Tmin_fixed,factor_rcool_fixed,rt_tmp,acool_fixed,aT_fixed,bT_fixed,cT_fixed);     
